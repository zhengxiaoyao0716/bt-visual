...
export default function LineRender({ index, total, width, height, anchors, onMoved, redrawSig, color }: Props) {
    const ref = useRef<HTMLAnchorElement>(null)
    const [refresh, setRefresh] = useState(0)

    useLayoutEffect(() => { // 这里需要等 react 绘制完毕后再同步调用，不能用 useEffect，否则连接线会闪烁
        ...
        const lineTo = createLineTo(`${(index - total / 2 + 0.5) / total * width}px`, `${height}px`)
    ...
    return (<Anchor ref={ref} className={grabableClass}>◉</Anchor>)
}
...
function findLineRoot(element: HTMLElement, anchor: HTMLAnchorElement): HTMLElement | null {
    if (element.classList.contains(lineToParentClass)) {
        if (element != anchor.parentElement) return element
    }
    if (element.parentElement == null || element.parentElement == element) return null
...
function getOrCreateLineSvg(root: HTMLElement): SVGSVGElement {
    for (const child of root.childNodes) {
        if (child instanceof SVGSVGElement && child.classList.contains('line-container')) {
