...
export module Composition {
    export interface Selector extends Composite {
        type: (
            | '?=Selector'      // 选择执行 // 依次执行 nodes 内的子节点，直到任意某个子节点执行成功，返回成功。当全部子节点执行失败时，返回失败。
        )
    }
    export interface Sequence extends Composite {
        type: (
            | '&=Sequence'      // 顺序执行 // 依次执行 nodes 内的子节点，直到任意某个子节点执行失败，返回失败。当全部子节点执行成功时，返回成功。
        )
    }
    export interface Parallel extends Composite {
        type: (
            | '>=Parallel'      // 并行执行 // 同时执行 nodes 内的子节点，待全部完成后根据执行成功的节点的数量，大于等于 expected 返回成功，否则失败。
        )
    ...
    export interface SelectBy extends Composite {
        type: (
            | '?:SelectBy'      // 条件选择 // 根据 index 给定的值，执行对应位置的节点
        )
        index: Store.Reader.Number      // 要执行的节点的位置，0-based
    }
    export interface SelectIf extends Composite {
        type: (
            | '?:SelectIf'      // 布尔选择 // 先执行 cond (nodes.first) 节点，cond 成功时执行 nodes.second，cond 失败时执行 nodes.third
        )
...
export module Decorator {
    ...
    export interface Reapeat extends Decorator {
        type: (
            | '@Repeat'         // 重复执行 node，直到成功次数达到 success 时返回成功，或失败次数达到 failure 时返回失败
        )
    ...
    export interface Delay extends Decorator {
        type: (
            | '@Delay'          // 延迟 millis 毫秒后执行 node 并返回 node 的执行结果
        )
    ...
    export interface Store extends Decorator {
        type: (
            | '@Store'          // 存储器，将 node 的执行结果存储到 store 中，返回 node 的执行结果
        )
...
export module Store {
    ...
    export module Reader {
        ...
        export type Number = number | (Reader<number> & { type: 'number' }) | Random
        export type String = string | (Reader<string> & { type: 'string', format?: | 'list' | 'dict' })
