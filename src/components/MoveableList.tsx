...
export function useMoveableList<T>(
    items: T[],
    appendItem: () => PromiseLike<T | null>,
    refreshList: () => void,
    renderItem: (item: T, index: number, showMenu: (event: MouseEvent<HTMLElement>) => void, anchor: JSX.Element) => React.ReactNode
) {
    ...
    const confirmRemoveProps = {
        cancel: trans('CANCEL'), submit: trans('REMOVE'),
    ...
    const listItems = items.map((item, index) => renderItem(
        item, index, showMenu.bind(null, index),
        < Stack >
            <ArrowDropUpIcon onClick={() => moveUp(index)} color={index <= 0 ? 'disabled' : 'inherit'} />
            <ArrowDropDownIcon onClick={() => moveDown(index)} color={index >= items.length - 1 ? 'disabled' : 'inherit'} />
        </Stack >
    ))
    ...
    const itemsMenu = (<>
        ...
        >
            ...
            <MenuItem onClick={moveTop} disabled={anchorEl && anchorEl[0] <= 0 ? true : false}>
                <ListItemIcon>
                    <VerticalAlignTopIcon />
                </ListItemIcon>
                <ListItemText>{trans('Move Top')}</ListItemText>
            </MenuItem>
            <MenuItem onClick={moveBottom} disabled={anchorEl && anchorEl[0] >= items.length - 1 ? true : false}>
                <ListItemIcon>
