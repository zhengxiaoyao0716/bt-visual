...
export interface DragState { left: number, top: number, dragging: boolean }
...
export function useDragMoving(before: BeforeHandler<React.MouseEvent> = defaultBeforeHandler): [DragListeners<React.MouseEvent>, DragState, Dispatch<DragState>] {
...
export function createDragListeners<E extends DragEvent>(state: DragState, setState: Dispatch<DragState>, before: BeforeHandler<E> = defaultBeforeHandler): DragListeners<E> {
    ...
    const onMouseMove = (event: E) => {
        ...
        if (draging) setState({ left: left + event.movementX, top: top + event.movementY, dragging: true })
    }
